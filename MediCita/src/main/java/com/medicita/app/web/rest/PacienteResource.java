...
/**
 * REST controller for managing {@link com.medicita.app.domain.Paciente}.
 */
@RestController
...
public class PacienteResource {
    ...
    private static final String ENTITY_NAME = "mediCitaPaciente";
    ...
    /**
     * {@code GET  /pacientes/keycloak/:keycloakId} : get the paciente by
     * keycloakId.
     *
     * @param keycloakId the keycloakId of the pacienteDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the pacienteDTO, or with status {@code 404 (Not Found)}.
     */
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_MEDICO', 'ROLE_USER')")
    @GetMapping("/keycloak/{keycloakId}")
    public ResponseEntity<PacienteDTO> getPacienteByKeycloakId(@PathVariable("keycloakId") String keycloakId) {
        LOG.debug("REST request to get Paciente by keycloakId : {}", keycloakId);
        Optional<PacienteDTO> pacienteDTO = pacienteService.findOneByKeycloakId(keycloakId);
        return ResponseUtil.wrapOrNotFound(pacienteDTO);
    }
